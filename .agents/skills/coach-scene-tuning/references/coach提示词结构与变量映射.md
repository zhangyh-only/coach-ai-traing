...
各场景正式 prompt 以场景目录里的 `最新使用提示词.md` 为准，受保护与否按仓内 guard。当前推荐结构（Bella v0.3 后沉淀）：
...
随机池 SSOT 随场景归属：场景1＝`场景1_质感自用Elena/随机机制/最新随机池内容.md`，场景2＝`场景2_奥莱私域轻奢Bella/随机机制/最新随机池内容.md`（均按仓内 guard 保护）。字段与后端代码 `NstrRoleplayCard` 对齐：`{name, content, enabled}`。
